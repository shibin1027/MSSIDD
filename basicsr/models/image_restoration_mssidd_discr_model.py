...
from basicsr.utils import get_root_logger, imwrite, tensor2img, tensor2img_ours
from basicsr.utils.dist_util import get_dist_info

loss_module = importlib.import_module('basicsr.models.losses')
metric_module = importlib.import_module('basicsr.metrics')

class ImageRestorationMSSIDDDiscrModel(BaseModel):
    """Base Deblur model for single image deblur."""

    def __init__(self, opt):
        super(ImageRestorationMSSIDDDiscrModel, self).__init__(opt)
    ...
    def feed_data(self, data, is_val=False):
        ...
        self.batch_size = B
        self.sensor_num = SN
        self.lq = self.lq.reshape(B*SN, *self.lq.shape[-3:])
        if 'gt' in data:
            self.gt = data['gt'].to(self.device)
            self.gt = self.gt.reshape(B*SN, *self.gt.shape[-3:])
        if 'var' in data:
    ...
    def dist_validation(self, dataloader, current_iter, tb_logger, save_img, rgb2bgr, use_image):
        ...
        raw_expand_ratio = self.opt['val'].get('raw_expand_ratio', 1.0)
        gamma_ratio = self.opt['val'].get('gamma_ratio', 1.0)
        if with_metrics:
            self.metric_results = {
                metric: 0
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            sr_img = tensor2img([visuals['result']], rgb2bgr=rgb2bgr, gamma=gamma_ratio, dgain=raw_expand_ratio)
            if 'gt' in visuals:
                gt_img = tensor2img([visuals['gt']], rgb2bgr=rgb2bgr, gamma=gamma_ratio, dgain=raw_expand_ratio)
                del self.gt
